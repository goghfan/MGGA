...
class TransMorphEncoder(nn.Module):
  def __init__(self, config):
    """
    只保留 TransMorph 的特征提取部分（编码器）。
    Args:
      config: 配置对象，包含模型的各种超参数。
    """
    super(TransMorphEncoder, self).__init__()
  ...
  def forward(self, x):
    ...
    前向传播函数。
    Args:
      x: 输入图像，形状为 (B, C, H, W, T)，其中 B 是 batch size，C 是通道数，H, W, T 是图像的空间维度。
    Returns:
      features: 编码器提取的多尺度特征，是一个列表，包含不同尺度的特征图。
    """
    # 通过编码器提取特征
...
class TransMorphDecoder(nn.Module):
    ...
    def forward(self, features):
        # features 是一个列表: [f4, f3, f2, f1, bottleneck/gpr_output]
        # 注意：MedSAMRegNet 里的调用是: 
        # flow = self.register_decoder(reg_features[:-1] + [fused_embedding])
        
        # TransMorphEncoder 输出的是: [f1, f2, f3, f4] (假设4层)
        # 这里的顺序非常重要，必须和 Encoder 输出对齐
        
        # 让我们重新梳理一下数据流：
        # Encoder (Swin) 输出 outs 列表，索引 0,1,2,3 对应 embed_dim * 1,2,4,8
        # outs[0]: Stage 1 (Embed*2)
        # outs[1]: Stage 2 (Embed*4)
        # outs[2]: Stage 3 (Embed*8) <-- Bottleneck
        # 等等，Swin 的层级通常是 96 -> 192 -> 384 -> 768
        
        x = features[-1] # 这是 GPR 处理后的 Bottleneck (768)
        
        # 对应 Encoder 的倒数第二层 (384)
        f1 = features[-2] if self.if_transskip else None 
        # 对应 Encoder 的倒数第三层 (192)
        f2 = features[-3] if self.if_transskip else None
        # 对应 Encoder 的倒数第四层 (96)
        f3 = features[-4] if self.if_transskip else None
        
        # Conv skip (如果用到的话)
        f4 = None 
        f5 = None
        
        x = self.up0(x, f1) # Input: 768 + 384 -> Output: 384
        x = self.up1(x, f2) # Input: 384 + 192 -> Output: 192
        x = self.up2(x, f3) # Input: 192 + 96  -> Output: 96
        x = self.up3(x, f4) # Input: 96 (+0)   -> Output: 48
        x = self.up4(x, f5) # Input: 48 (+0)   -> Output: 16
        
        flow = self.reg_head(x)
        return flow

if __name__ == '__main__':
  # 假设 config 是一个配置对象，包含 Swin Transformer 的超参数
  # config = {
  #   "patch_size": 4,
  #   "in_chans": 2, # 输入通道数（例如源图像和目标图像）
  #   "embed_dim": 96,
  #   "depths": [2, 2, 6, 2],
  #   "num_heads": [3, 6, 12, 24],
  #   "window_size": (7, 7, 7),
  ...
  def get_3DTransMorph_config():
    ...
    config.img_size = (128,512,512)
    return config
    # 创建模型
  def test_Encoder():
    config = get_3DTransMorph_config()

    model = TransMorphEncoder(config)

    # 输入图像
    x = torch.randn(1, 2, 128 ,512, 512) # (B, C, H, W, T)
    ...
    '''
    Feature 0 shape: torch.Size([1, 64, 32, 128, 128])
    Feature 1 shape: torch.Size([1, 128, 16, 64, 64])
    Feature 2 shape: torch.Size([1, 256, 8, 32, 32])
    '''
  def test_Decoder():
    config = get_3DTransMorph_config()
    model = TransMorphDecoder(config)
    x = torch.randn(1, 256, 8, 32, 32) # (B, C, H, W, T)
    y = torch.randn(1, 128, 16, 64, 64) # (B, C, H, W, T)
    z = torch.randn(1, 64, 32, 128, 128) # (B, C, H, W, T)
    input = [z,y,x]
    flow = model(input)
    print(flow.shape)
