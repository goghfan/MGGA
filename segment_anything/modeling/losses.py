...
class DiceLoss(nn.Module):
  ...
  def forward(self, y_pred, y_true):
    ...
    y_pred = nn.functional.one_hot(y_pred.long(),num_classes=self.num_class)
    y_pred = torch.squeeze(y_pred, 1)
    y_pred = y_pred.permute(0, 4, 1, 2, 3).contiguous().float()
    y_true = nn.functional.one_hot(y_true, num_classes=self.num_class)
    y_true = torch.squeeze(y_true, 1)
    y_true = y_true.permute(0, 4, 1, 2, 3).contiguous().float()
